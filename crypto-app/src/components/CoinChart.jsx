...
const CoinChart = ({currency}) => {
    ...
    const CoinChartData=async()=>{
      console.log('CoinChartData function is called'); // Log to check if the function is triggered
      try {
        const { data } = await axios.get(`${Baseurl}/coins/${id}/market_chart?vs_currency=${currency}&days=${days}`);
        console.log('Fetched Data:', data); // Log the data to see the response
    ...
    useEffect(() => {
      console.log('useEffect triggered:', { currency, id, days });
      CoinChartData();
    }, [currency, id, days]);
